...
from captum.attr import visualization as viz  # <-- IMPORT MANQUANT
...
def main():
    ...
    # TODO: Charger le modèle pour la classification d'images
    ...
    # Couche cible (dernière couche conv du dernier stage)
    ...
    fig, axis = viz.visualize_image_attr(
        ...
        sign="positive",  # Grad-CAM = influences positives (ReLU)
        show_colorbar=True,
        title=f"Grad-CAM - Pred: {hf_model.config.id2label[predicted_class_idx]}"
    )

    output_filename = f"gradcam_{image_path.rsplit('.', 1)[0]}.png"
